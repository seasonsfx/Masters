#include "plugins/testfilter/testfilter.h"
#include <QDebug>
#include <QEvent>
#include <QKeyEvent>
#include <QAction>
...
QString Testfilter::getName(){
    return "testfilter Tool";
}

void Testfilter::initialize(Core *core){
    core_= core;
    cl_ = core_->cl_;
    ll_ = core_->ll_;
    ...
    enable_ = new QAction(QIcon(":/testfilter.png"), "Enable testfilter", 0);
...
void Testfilter::select2D(int x, int y){
...
int Testfilter::select3D(float x, float y){
...
bool Testfilter::mouseClickEvent(QMouseEvent * event){
    return true;
}

bool Testfilter::mouseMoveEvent(QMouseEvent * event) {
...
bool Testfilter::mousePressEvent(QMouseEvent * event) {
...
bool Testfilter::mouseReleaseEvent(QMouseEvent * event){
    core_->us_->endMacro();
    last_mouse_pos_ << event->x(), event->y();
...
Q_PLUGIN_METADATA(IID "za.co.circlingthesun.cloudclean.testfilter")
